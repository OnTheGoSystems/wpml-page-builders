...
class Test_WPML_PB_String_Translation extends WPML_PB_TestCase {
	 ...
	 * @test
	 */
	public function it_should_allow_override_of_package_handling() {
		$kind = 'abc';
		$strategyKind = 'xyz';

		$updater = \Mockery::mock( 'WPML_PB_XYZ_Updater' );
		$updater->shouldReceive( 'update' )->once();

		$strategy = \Mockery::mock( 'WPML_PB_Shortcode_Strategy' );
		$strategy->shouldReceive( 'get_package_kind' )->andReturn( $strategyKind );
		$strategy->shouldReceive( 'get_update_post' )->andReturn( $updater );

		$subject = new WPML_PB_String_Translation_By_Strategy(
			\Mockery::mock( 'wpdb' ),
		...
		$subject->add_package_to_update_list( $package, 'de' );

		// Test that the filter can force the update
		\WP_Mock::onFilter( 'wpml_pb_should_handle_package' )->with( false, $kind, $strategyKind )->reply( true );

		$subject->save_translations_to_post();
